...
class TestHeaderClassifier:
    ...
    def create_test_element(self, text: str, bbox: Dict[str, float], page_number: int = 1, 
                           confidence: float = 0.85) -> DocumentElement:
    ...
    def create_page_context(self, page_width: float = 612.0, page_height: float = 792.0) -> Dict[str, Any]:
    ...
    def test_classifies_colon_ended_text_as_heading(self):
        ...
        for text in test_cases:
            element = self.create_test_element(
                text=text,
                bbox={'x0': 50.0, 'y0': 600.0, 'x1': 150.0, 'y1': 620.0}
            )
    ...
    def test_classifies_mixed_case_structural_text_as_heading(self):
        ...
        for text in test_cases:
            element = self.create_test_element(
                text=text,
                bbox={'x0': 50.0, 'y0': 650.0, 'x1': 250.0, 'y1': 670.0}
            )
    ...
    def test_page_position_affects_classification(self):
        ...
        middle_element = self.create_test_element(
            text=text,
            bbox={'x0': 50.0, 'y0': 400.0, 'x1': 200.0, 'y1': 420.0}  # Middle of page
        )
    ...
    def test_does_not_classify_person_names_as_heading(self):
        ...
        for text in test_cases:
            element = self.create_test_element(
                text=text,
                bbox={'x0': 150.0, 'y0': 600.0, 'x1': 300.0, 'y1': 620.0}
            )
    ...
    def test_does_not_classify_numeric_values_as_heading(self):
        ...
        for text in test_cases:
            element = self.create_test_element(
                text=text,
                bbox={'x0': 150.0, 'y0': 500.0, 'x1': 250.0, 'y1': 520.0}
            )
    ...
    def test_does_not_classify_common_form_values_as_heading(self):
        ...
        for text in test_cases:
            element = self.create_test_element(
                text=text,
                bbox={'x0': 200.0, 'y0': 450.0, 'x1': 350.0, 'y1': 470.0}
            )
    ...
    def test_does_not_classify_long_descriptive_text_as_heading(self):
        ...
        for text in test_cases:
            ...
            is_heading = classify_as_heading(element, page_context)
            assert not is_heading, f"Long text should NOT be classified as heading (length: {len(text)})"
    ...
    def test_handles_edge_cases_gracefully(self):
        ...
        for text, expected_result, description in edge_cases:
            if not text.strip():  # Handle empty/whitespace
                ...
                element = self.create_test_element(
                    text=text,
                    bbox={'x0': 50.0, 'y0': 400.0, 'x1': 100.0, 'y1': 420.0}
                )
                
                is_heading = classify_as_heading(element, page_context)
                assert is_heading == expected_result, f"{description}: expected {expected_result}, got {is_heading}"
            else:
                element = self.create_test_element(
                    text=text,
                    bbox={'x0': 50.0, 'y0': 400.0, 'x1': 200.0, 'y1': 420.0}
                )
    ...
    def test_international_names_and_addresses(self):
        ...
        for text in test_cases:
            element = self.create_test_element(
                text=text,
                bbox={'x0': 150.0, 'y0': 500.0, 'x1': 300.0, 'y1': 520.0}
            )
            
            is_heading = classify_as_heading(element, page_context)
            assert not is_heading, f"'{text}' should NOT be classified as heading (international name/address)"
    ...
    def test_mixed_alphanumeric_content(self):
        ...
        for text, expected_result, description in test_cases:
            element = self.create_test_element(
                text=text,
                bbox={'x0': 50.0, 'y0': 600.0, 'x1': 200.0, 'y1': 620.0}
            )
    ...
    def test_borderline_cases(self):
        ...
        for text, description in borderline_cases:
            element = self.create_test_element(
                text=text,
                bbox={'x0': 50.0, 'y0': 500.0, 'x1': 150.0, 'y1': 520.0}
            )
            ...
            is_heading = classify_as_heading(element, page_context)
            assert isinstance(is_heading, bool), f"classify_as_heading should return boolean for '{text}'"
