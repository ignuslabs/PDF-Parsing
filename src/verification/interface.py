...
class VerificationInterface:
    ...
    def migrate_verification_states(self):
        """Migrate existing verification states to include new attributes.

        This method ensures backward compatibility with VerificationState objects
        that were created before the corrected_text and verified_at attributes
        were added to the model.
        """
        for element_id, state in self.verification_states.items():
            # Check if state has all required attributes, add if missing
            if not hasattr(state, "corrected_text"):
                state.corrected_text = None
            if not hasattr(state, "verified_at"):
    ...
    def _init_verification_states(self):
        ...
        for i, element in enumerate(self.elements):
            # Create VerificationState with only required parameters
            state = VerificationState(element_id=i, status="pending", timestamp=datetime.now())
            # Set optional attributes explicitly
            ...
            self.verification_states[i] = state
    ...
    def mark_element_correct(self, element_id: int, verified_by: Optional[str] = None):
        ...
        state.corrected_text = None  # Clear corrected text
        state.verified_at = datetime.now()  # Record when verification was completed
    ...
    def mark_element_incorrect(
        ...
        Args:
            ...
            correction: Corrected text content (legacy parameter)
            corrected_text: Corrected text content
            notes: Additional notes about the correction
            verified_by: User ID who verified the element
        ...
        state.timestamp = datetime.now()
        state.verified_by = verified_by or "system"  # Default to "system" if not provided
        state.correction = correction or corrected_text  # Legacy support
        state.corrected_text = corrected_text or correction
        state.notes = notes
        state.verified_at = datetime.now()

        self._add_history_entry(
            element_id, "marked_incorrect", state.verified_by, {"correction": correction}
        )
    ...
    def mark_element_partial(
        ...
        Args:
            ...
            corrected_text: Partially corrected text content
            verified_by: User ID who verified the element
        ...
        self._add_history_entry(
            ...
            "marked_partial",
            verified_by,
            {"notes": notes, "corrected_text": corrected_text},
        )
    ...
    def get_verification_summary(self) -> Dict[str, Union[int, float]]:
        ...
        # Count total corrections (use getattr for backward compatibility)
        total_corrections = sum(
            1
            for state in self.verification_states.values()
            if getattr(state, "corrected_text", None) is not None
        )

        return {
            # New field names for UI
    ...
    def get_verification_summary_by_page(self) -> Dict[int, Dict[str, Union[int, float]]]:
        ...
        for page_summary in page_summaries.values():
            ...
            page_summary["accuracy"] = page_summary["correct"] / verified if verified > 0 else 0.0
    ...
    def get_verification_summary_by_type(self) -> Dict[str, Dict[str, Union[int, float]]]:
        ...
        for type_summary in type_summaries.values():
            ...
            type_summary["accuracy"] = type_summary["correct"] / verified if verified > 0 else 0.0
    ...
    def get_verification_progress(self) -> Dict[str, Union[int, float]]:
        ...
        total = len(self.verification_states)
        pending_count = sum(
            1 for state in self.verification_states.values() if state.status == "pending"
        )
        ...
        return {
            ...
            "completion_percentage": (verified_count / total * 100.0) if total > 0 else 0.0,
        }
    ...
    def get_elements_needing_verification(
        ...
        for element_id, element in enumerate(self.elements):
            ...
            if state.status == "pending" or element.confidence < confidence_threshold:
                needing_verification.append((element_id, element))

        return needing_verification

    def export_verification_data(self, format: str = "json", corrections_only: bool = False) -> str:
