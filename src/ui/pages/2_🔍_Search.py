...
def perform_search(
    query: str, search_mode: str, filters: Dict[str, Any]
    ...
    for i, (result, doc_index) in enumerate(results):
        ...
        with st.expander(
            ...
            with col2:
                ...
                if st.button(
                    f"Go to Verify",
                    key=f"verify_{doc_index}_{element.metadata.get('element_id', i)}",
                ):
...
def highlight_matches(text: str, query: str) -> str:
    ...
    try:
        ...
        highlighted = pattern.sub(
            lambda m: f'<mark style="background-color: yellow;">{m.group()}</mark>', text
        )
