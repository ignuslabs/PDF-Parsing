...
try:
    from src.core.models import DocumentElement, ParsedDocument, VerificationState
...
def export_to_json(documents: List[ParsedDocument], options: Dict[str, Any]) -> str:
    ...
    for i, doc in enumerate(documents):
        doc_data = {"document_id": i, "elements": []}
        ...
        for element in doc.elements:
            ...
            if verification_interface and options["content"]["verification"]:
                ...
                element_data["verification"] = {
                    ...
                    "verified_at": (
                        verification_state.verified_at.isoformat()
                        if verification_state.verified_at
                        else None
                    ),
...
def export_to_markdown(documents: List[ParsedDocument], options: Dict[str, Any]) -> str:
    ...
    for doc_idx, doc in enumerate(documents):
        ...
        for element in sorted(
            doc.elements, key=lambda x: (x.page_number, x.bbox["y0"] if x.bbox else 0)
        ):
            # Skip if corrections_only and no corrections
...
def export_to_html(documents: List[ParsedDocument], options: Dict[str, Any]) -> str:
    ...
    for doc_idx, doc in enumerate(documents):
        ...
        if options["content"]["metadata"]:
            ...
            html_content.append(
                f"            <p><strong>Elements:</strong> {len(doc.elements)}</p>"
            )
            html_content.append(
                f"            <p><strong>Pages:</strong> {doc.metadata.get('page_count', 'Unknown')}</p>"
            )
            html_content.append(
                f"            <p><strong>Size:</strong> {doc.metadata.get('file_size', 0) / 1024:.1f} KB</p>"
            )
        ...
        for element in sorted(
            ...
            html_content.append(
                f"            <div class='element-type'>{element.element_type.title()}</div>"
            )
            ...
            if verification_interface and options["content"]["verification"]:
                ...
                if verification_state.corrected_text:
                    html_content.append(
                        f"            <p><strong>Original:</strong> {element.text}</p>"
                    )
                    html_content.append(
                        f"            <p><strong>Corrected:</strong> {verification_state.corrected_text}</p>"
                    )
                else:
                ...
                if verification_state.notes:
                    html_content.append(
                        f"            <div class='verification'>Notes: {verification_state.notes}</div>"
                    )
            else:
                html_content.append(f"            <p>{element.text}</p>")
...
def display_analytics():
    ...
    total_pages = sum(
        doc.metadata.get("page_count", 0) for doc in st.session_state.parsed_documents
    )
