    ...
    with st.expander("Advanced Settings"):
        ...
        if st.session_state.parser_config['enable_ocr']:
            st.session_state.parser_config['ocr_language'] = st.text_input(
                ...
                help="Language codes (e.g., 'eng', 'fra', 'deu'). Multiple languages: 'eng+fra'"
            )
...
def display_parsing_results(parsed_doc: ParsedDocument):
    ...
    with col4:
        avg_confidence = sum(e.confidence for e in parsed_doc.elements) / len(parsed_doc.elements) if parsed_doc.elements else 0
                ...
                for i, uploaded_file in enumerate(valid_files):
                    ...
                    try:
                        ...
                        if parsed_doc:
                            ...
                            status_text.success(f"✅ Parsed {uploaded_file.name} in {parsing_time:.1f}s")
                        else:
