...
class ResultsDisplay:
    ...
    def display_parsing_summary(self, parsed_doc, show_details: bool = True):
        ...
        with col4:
            if parsed_doc.elements:
                avg_confidence = sum(e.confidence for e in parsed_doc.elements) / len(parsed_doc.elements)
    ...
    def display_element_list(
        ...
        if len(filtered_elements) > max_display:
            ...
            st.write(f"Showing {start_idx + 1}-{min(end_idx, len(filtered_elements))} of {len(filtered_elements)} elements")
        else:
