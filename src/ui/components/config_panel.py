...
class ParserConfigPanel:
        ...
        with st.expander("🔧 Advanced Settings"):
            ...
            with col2:
                ...
                if config.get('enable_ocr', False) and config.get('generate_page_images', True):
                    st.warning("⚠️ OCR + Page Images uses more memory. Consider reducing image scale for large documents.")
    ...
    def display_performance_indicators(self, config: Dict[str, Any]):
        ...
        with col1:
            st.metric(
                "Speed",
                f"{speed_score}/100",
                help="Estimated processing speed"
            )
        ...
        with col2:
            st.metric(
                "Quality",
                f"{quality_score}/100",
                help="Estimated extraction quality"
            )
        ...
        with col3:
            st.metric(
                "Memory Usage",
                f"{memory_usage}/100",
                help="Estimated memory consumption"
            )
        
        # Recommendations
        if speed_score < 30:
            st.warning("⚠️ **Slow configuration detected.** Consider disabling OCR or using fast table mode for better speed.")
        
        if memory_usage > 70:
            st.warning("⚠️ **High memory usage.** Consider reducing image scale or disabling page image generation for large documents.")
    ...
    def validate_config(self, config: Dict[str, Any]) -> List[str]:
        ...
        if not config.get('generate_page_images', True):
            st.info("💡 **Note:** Page images are disabled. Verification functionality will be limited.")
