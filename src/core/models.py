...
class ParsedDocument:
    ...
    def export_to_html(self) -> str:
        ...
        # Export each page
        for page_num in sorted(elements_by_page.keys()):
            html_lines.extend([f"<div class='page'>", f"<h2>Page {page_num}</h2>"])
            ...
            for element in page_elements:
                ...
                if element.element_type == "heading":
                    html_lines.append(
                        f"<div class='element {element_class}'><h3>{escaped_text}</h3></div>"
                    )
                elif element.element_type == "table":
                    html_lines.extend(
                        [
                    ...
                    html_lines.append(
                        f"<div class='element {element_class}'><pre>{escaped_text}</pre></div>"
                    )
                else:
                    html_lines.append(
                        f"<div class='element {element_class}'><p>{escaped_text}</p></div>"
                    )
