...
from src.utils.logging_config import time_it, log_memory_usage

# Initialize logger for search engine
...
class SmartSearchEngine:
    ...
    def search(
        ...
        **kwargs,
        ...
        if region:
            candidate_elements = self.filter_by_region(
                candidate_elements, region, strict_containment
            )
        ...
        if enable_fuzzy:
            fuzzy_results = self._fuzzy_search(
                query, candidate_elements, fuzzy_threshold, include_metadata
            )
    ...
    def search_exact(self, query: str, **kwargs) -> List[SearchResult]:
        ...
        """
        return self.search(query, enable_fuzzy=False, **kwargs)
    ...
    def search_fuzzy(self, query: str, threshold: float = 0.8, **kwargs) -> List[SearchResult]:
        ...
        Returns:
            List of SearchResult objects with fuzzy matches
        """
        return self.search(query, enable_fuzzy=True, fuzzy_threshold=threshold, **kwargs)

    def search_semantic(self, query: str, **kwargs) -> List[SearchResult]:
        """Perform semantic search (currently same as fuzzy, can be enhanced with embeddings).
        ...
        Returns:
            List of SearchResult objects with semantic matches
        """
        # For now, semantic search is implemented as enhanced fuzzy search
        # In future, this could use embeddings/vector search
        return self.search(query, enable_fuzzy=True, fuzzy_threshold=0.6, **kwargs)

    def _exact_search(
        self, query: str, elements: List[DocumentElement], include_metadata: bool = False
    ) -> List[SearchResult]:
        ...
        for element in elements:
            ...
            if query_normalized in text_normalized:
                ...
                best_match = SearchResult(
                    element=element, score=score, match_type="exact", matched_text=matched_text
                )
                best_score = score
            ...
            if include_metadata and element.metadata:
                for key, value in element.metadata.items():
                    ...
                    if query_normalized in key_normalized:
                        score = self._calculate_exact_score(
                            query_normalized, key_normalized, element
                        )
                    ...
                    if isinstance(value, str):
                        ...
                        if query_normalized in meta_normalized:
                            score = self._calculate_exact_score(
                                query_normalized, meta_normalized, element
                            )
    ...
    def _calculate_exact_score(
        self, query_normalized: str, text_normalized: str, element: DocumentElement
    ) -> float:
        ...
        final_score = self._calculate_relevance_score(
            element, query_normalized, "exact", base_score
        )
    ...
    def _fuzzy_search(
        ...
        for element in elements:
            ...
            combined_score = token_ratio * 0.4 + partial_ratio * 0.3 + weighted_ratio * 0.3
            ...
            if include_metadata and element.metadata:
                for key, value in element.metadata.items():
                    ...
                    if isinstance(value, str):
                        ...
                        # Combined score with weights
                        combined_score = (
                            token_ratio * 0.4 + partial_ratio * 0.3 + weighted_ratio * 0.3
                        )
    ...
    def _calculate_relevance_score(
        self, element: DocumentElement, query: str, match_type: str, base_score: float
    ) -> float:
    ...
    def filter_by_element_types(
        self, elements: List[DocumentElement], element_types: List[str]
    ...
    def filter_by_pages(
        self, elements: List[DocumentElement], page_numbers: List[int]
    ) -> List[DocumentElement]:
    ...
    def filter_by_region(
        ...
        for element in elements:
            ...
            if strict_containment:
                ...
                if (
                    element_bbox["x0"] >= region["x0"]
                    and element_bbox["y0"] >= region["y0"]
                    and element_bbox["x1"] <= region["x1"]
                    and element_bbox["y1"] <= region["y1"]
                ):
    ...
    def _bboxes_overlap(self, bbox1: Dict[str, float], bbox2: Dict[str, float]) -> bool:
        ...
        """
        return not (
            bbox1["x1"] < bbox2["x0"]  # bbox1 is left of bbox2
            or bbox2["x1"] < bbox1["x0"]  # bbox2 is left of bbox1
            or bbox1["y1"] < bbox2["y0"]  # bbox1 is above bbox2
            or bbox2["y1"] < bbox1["y0"]  # bbox2 is above bbox1
        )

    def filter_by_metadata(
        self, elements: List[DocumentElement], metadata_filters: Dict[str, Any]
    ) -> List[DocumentElement]:
        ...
        try:
            ...
            for i, element in enumerate(self.elements):
                ...
                similarity = self._calculate_cosine_similarity(
                    query_embedding[0], element_embedding
                )
